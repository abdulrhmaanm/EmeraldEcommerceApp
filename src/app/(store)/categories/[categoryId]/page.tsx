...
export default async function CategoryDetailsPage({
  ...
  const filtered = products.filter((p: IProduct) => (p.category as unknown as { _id?: string })?._id === categoryId);
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="bg-emerald-900 text-white">
        <div className="container mx-auto px-4 py-16 flex flex-col md:flex-row items-center gap-8">
          <div className="relative w-40 h-40 rounded-2xl overflow-hidden border-4 border-emerald-700 flex-shrink-0 bg-emerald-800">
            <Image
              src={category.image}
              alt={category.name}
              fill
          ...
          <div>
            <p className="text-emerald-300 text-sm font-bold uppercase tracking-widest mb-2">Category</p>
            <h1 className="text-4xl lg:text-5xl font-extrabold mb-3">{category.name}</h1>
      ...
      <div className="container mx-auto px-4 py-12">
        {filtered.length === 0 ? (
          <div className="text-center py-24 text-gray-400 text-lg">No products in this category yet.</div>
        ) : (
